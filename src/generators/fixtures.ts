...
function generatePlaywrightConfig(config: QAAgentConfig): string {
  ...
  lines.push('');
  lines.push(`// Wipe screenshots before each run — only latest survive`);
  lines.push(`// HTML report is generated fresh by Playwright each run`);
  ...
  lines.push(`  reporter: [`);
  lines.push(`    ['list'],`);
  lines.push(`    ['html', { open: 'never', outputFolder: './html-report' }],`);
